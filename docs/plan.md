...
| API 호환성 검사(japicmp), NullAway, SpotBugs | 첫 릴리스 이후 | 💡 |
...
- **린터 — Error Prone**
  - 컴파일 중 실제 버그(자원 누수, `==` 오용, null 흐름, 자기대입 등)를 잡음. SelfAssignment 탐지 검증 완료.
...
└── framework/
    ├── build.gradle         # Java21 + Spotless + Error Prone + 테스트
    └── src/
...
- 💡 **NullAway / SpotBugs** — null 안전성·정적분석 보강.
- 💡 **CI** — `spotlessCheck` + `test` + `build` 를 PR에서 자동 검증.
