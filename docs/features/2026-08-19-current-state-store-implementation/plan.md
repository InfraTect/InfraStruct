...
| JSON | 결과 |
|---|---|
| 소수부 없음 + `int` 범위 | `Integer` |
| 소수부 없음 + `int` 범위 밖 | `Long` |
| 소수부 있음 | `Double` |

`int` 를 우선하는 이유: `config` 는 사용자의 자원 클래스 필드를 스캔해 만들어지고, 그 필드는
대부분 `int` 다. 가장 흔한 경우를 정확히 맞춘다. Gson 은 `Integer 22` 를 `22` 로,
`Double 22.0` 을 `22.0` 으로 쓰므로 **정수와 실수는 파일 내용만으로 구분된다.**

**남는 한계 (숨기지 않고 기록한다)**: `long sizeGb = 100` 처럼 `long` 인데 값이 작으면
읽을 때 `Integer 100` 이 된다. 값은 같지만 타입이 달라 `Object.equals` 로 비교하면 다르다고
나온다. 완전한 타입 보존(`{"t":"long","v":100}` 같은 형태)은 **하지 않는다** — 사람이 읽을 수
있는 상태 파일이라는 가치를 잃는 값에 비해 이득이 작다.

**그래서 이 한 가지는 뒤 단계에 넘긴다**: **Comparator 는 `config` 값을 비교할 때 숫자를
타입이 아니라 값으로 비교해야 한다.** §4.1 에서는 "상태 저장소가 뒤 단계를 제약하지 않는다"를
원칙으로 삼았지만, 여기서는 JSON 이라는 매체의 한계라 피할 수 없다. summary 에 명시해
Comparator 를 맡는 쪽에 전달한다.

**저장소가 보장하는 것 / 못 하는 것**을 분명히 해 둔다:

- 보장한다 — `save` → `load` 왕복이 **안정적**이다. 여러 번 반복해도 값이 계속 변하지 않는다.
- 보장 못 한다 — 스캐너가 만든 `Long 100` 과 파일에서 온 `Integer 100` 이 `equals` 인 것.
  그건 Comparator 의 값 비교 정책이다.
...
3. **숫자는 왕복에서 Java 타입이 보존되지 않는다.** 읽을 때 `Integer`/`Long`/`Double` 로
   정규화하지만 `long` 인데 값이 작으면 `Integer` 로 돌아온다
